...
function withSessionDirectory(
  ...
  const sessionDir = mkdtempSync(join(tmpdir(), "moros-session-search-"));
...
test("finds text matches case-insensitively and reports a snippet", () => withSessionDirectory(async (sessionDir) => {
  ...
  writeSession(first, [
    sessionLine("user", "Please refactor the repository module and run its tests."),
  ]);
  writeSession(second, [
    sessionLine("assistant", [{ type: "text", text: "No matching module is mentioned here." }]),
  ]);
  ...
  const matches = await searchSessionContentFiles("REPOSITORY module", sessions, { sessionDir });

  assert.equal(matches.length, 1);
  assert.equal(matches[0].id, "first");
  assert.equal(matches[0].path, first);
  assert.match(matches[0].snippet, /repository module/i);
}));
