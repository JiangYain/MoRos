...
Moros can run Streamdown 2.5.0 on its current React 19 / Electron renderer. The
...
- The npm `latest` release was `streamdown@2.5.0` on the audit date. The tagged
  ...
  declares React and React DOM `^18.0.0 || ^19.0.0` peers; Moros currently uses
  React 19.2.7.
- Streamdown is an ESM package exporting the component and `styles.css`. It does
  not declare a Node engine floor in the 2.5.0 manifest. Moros uses it only in
...
- The 2.5.0 public type and implementation also accept `stagger`; its source
  default is 40 ms per new tokenized segment. Moros pins `stagger: 0`, so all
...
- Streamdown's default rehype pipeline enables raw HTML, sanitization, and
  hardening. Moros intentionally supplies only the tagged `sanitize` and
...
- The official installation guide assumes Tailwind scanning and shadcn-style
  design tokens. Moros does not add Tailwind; it supplies its existing
...
- The base 2.5.0 package manifest still lists Mermaid as a hard install
  dependency even though diagram rendering is plugin-driven. Moros does not
...
## Current Moros integration observed

- `package.json` pins `streamdown` exactly at `2.5.0` and removes direct
  `react-markdown` / `remark-gfm` declarations.
- `Markdown.tsx` preserves Moros code language tags, localized copy buttons,
