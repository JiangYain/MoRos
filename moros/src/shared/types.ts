...
export interface UiModel {
  ...
  name: string;
  reasoning: boolean;
  /** Exact reasoning levels accepted by this model after provider mapping. */
...
export interface UiSessionInfo {
  path: string;
  id: string;
  /** Working directory where the session was created; null for legacy sessions without one. */
  cwd: string | null;
  name?: string;
  firstMessage: string;
  createdAt: number;
...
export interface AgentStats {
  ...
  sessionPath?: string;
  sessionName?: string;
  /** Effective workspace owned by the active session. */
  workspaceDir: string;
  model?: {
    provider: string;
    id: string;
...
export type DependencyCategory = "runtime";
...
/** Operations implemented by the local Moros backend, excluding renderer-only event/window hooks. */
export type MorosBackendApi = Omit<
  MorosApi,
