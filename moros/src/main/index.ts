...
import { createMorosBackendApi } from "./moros-api";
...
const userDataOverride = process.env.MOROS_USER_DATA_DIR?.trim();
if (userDataOverride) {
...
if (process.platform === "win32") {
  app.setAppUserModelId("com.moros.desktop");
}
...
function createWindow(rendererUrl: string): BrowserWindow {
  ...
  const window = new BrowserWindow({
    ...
    webPreferences: {
      ...
      sandbox: false,
      // A background agent run keeps streaming while the window sits behind the
      // browser or another app. Chromium would otherwise throttle timers and
      // stop the frame clock for an occluded window, stranding queued deltas
      // and freezing the thread mid-response.
  ...
  void observeInitialNavigation(window.loadURL(rendererUrl), (error) => {
    console.error(`Failed to load the Moros renderer at ${rendererUrl}:`, error);
    if (!window.isDestroyed()) window.destroy();
  });

  // Temporary visual verification hook: capture the renderer at a few settled moments.
  if (process.env.MOROS_CAPTURE) {
    const outDir = process.env.MOROS_CAPTURE;
...
async function startApplication(): Promise<void> {
  ...
  console.log(`[Moros] Web app: ${rendererUrl}`);
  if (development) console.log(`[Moros] Web API: ${webServer.url}`);
...
void app
  ...
  .catch(async (error: unknown) => {
    ...
    console.error("Failed to start Moros:", error);
    await shutdown();
    if (shouldShowStartupErrorDialog(process.env)) {
      dialog.showErrorBox("Moros 启动失败", message);
      app.quit();
    } else {
...
app.on("window-all-closed", () => {
  void shutdown()
    .catch((error: unknown) => {
      console.error("Failed to shut down Moros cleanly:", error);
    })
    .finally(() => app.quit());
});
