...
export function Thread(): React.JSX.Element {
  const { t } = useI18n();
  const thread = useMoros((state) => state.thread) ?? [];
  const agentStreaming = useMoros((state) => state.streaming);
  const approvals = useMoros((state) => state.approvals) ?? [];
  const sessionId = useMoros((state) => state.stats?.sessionId);
