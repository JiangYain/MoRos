...
export function ProfileSettings(): React.JSX.Element {
  const { t } = useI18n();
  const profileAvatar = useMoros((state) => state.profileAvatar);
  const setProfileAvatar = useMoros((state) => state.setProfileAvatar);
  const setError = useMoros((state) => state.setError);
  const profileName = useMoros((state) => state.profileName);
  const profileHandle = useMoros((state) => state.profileHandle);
  const setProfileIdentity = useMoros((state) => state.setProfileIdentity);
