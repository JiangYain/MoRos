...
function ProviderRow({ provider }: { provider: UiProviderStatus }): React.JSX.Element {
  const { t } = useI18n();
  const setApiKey = useMoros((state) => state.setApiKey);
  const loginProvider = useMoros((state) => state.loginProvider);
  const removeApiKey = useMoros((state) => state.removeApiKey);
...
export function ModelsSettings(): React.JSX.Element {
  const { t } = useI18n();
  const models = useMoros((state) => state.models);
  const providers = useMoros((state) => state.providers);
  const settings = useMoros((state) => state.settings);
  const stats = useMoros((state) => state.stats);
  const setModel = useMoros((state) => state.setModel);
  const setModelEnabled = useMoros((state) => state.setModelEnabled);
  const setSummaryModel = useMoros((state) => state.setSummaryModel);
