...
import { ignoreCommandFailure, type SettingsNavigationGuard, useMoros } from "../../store";
...
function QuickPromptSettings(): React.JSX.Element {
  ...
  const setQuickPrompts = useMoros((state) => state.setQuickPrompts);
  const registerSettingsGuard = useMoros((state) => state.registerSettingsGuard);
  const unregisterSettingsGuard = useMoros((state) => state.unregisterSettingsGuard);
...
function AboutSettings(): React.JSX.Element {
  const { t } = useI18n();
  const version = useMoros((state) => state.version);
  return (
    <section className="settings-section-block" id="settings-about">
      ...
      <div className="settings-card">
        <div className="settings-language-row">
          <div className="settings-language-copy"><strong>Moros</strong><span>{t("settings.aboutDescription")}</span></div>
...
export function GeneralSettings(): React.JSX.Element {
  const { language, t } = useI18n();
  const commandLanguage = useMoros((state) => state.settings?.commandExplanationLanguage ?? "auto");
  const composerSendKey = useMoros((state) => state.settings?.composerSendKey ?? "enter");
  const setLanguage = useMoros((state) => state.setLanguage);
  const setCommandLanguage = useMoros((state) => state.setCommandExplanationLanguage);
  const setComposerSendKey = useMoros((state) => state.setComposerSendKey);
