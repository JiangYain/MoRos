...
import { type SettingsSection, useMoros } from "../store";
...
function buildSearchTargets(t: ReturnType<typeof useI18n>["t"]): SettingsSearchTarget[] {
  return [
    ...
    { sectionId: "dependencies", targetId: "settings-page-dependencies", title: t("settings.nav.dependencies"), description: t("settings.nav.dependenciesDescription"), keywords: "dependency runtime git bash shell 依赖 运行环境" },
    ...
    { sectionId: "appearance", targetId: "settings-code-font", title: t("settings.codeFont"), description: t("settings.codeFontDescription"), keywords: "font code mono Google Sans Code NF Maple Mono NF CN 字体 代码字体 等宽" },
    ...
    { sectionId: "general", targetId: "settings-about", title: t("settings.about"), description: t("settings.aboutDescription"), keywords: "about version moros build 关于 版本" },
  ];
}
...
export function SettingsWorkspace(): React.JSX.Element {
  const { t } = useI18n();
  const section = useMoros((state) => state.settingsSection) ?? "general";
  const openSettings = useMoros((state) => state.openSettings);
  const closeSettings = useMoros((state) => state.closeSettings);
  const pendingNavigation = useMoros((state) => state.pendingSettingsNavigation);
  const settingsGuard = useMoros((state) => state.settingsGuard);
  const resolveSettingsNavigation = useMoros((state) => state.resolveSettingsNavigation);
