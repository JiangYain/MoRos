...
const WEB_MESSAGES: Record<AppLanguage, Record<WebMessageKey, string>> = {
  "zh-CN": {
    apiUnavailable: "Moros Web API 暂不可用。",
    speechUnsupported: "当前浏览器不支持语音识别，请使用最新版 Chrome 或桌面版。",
    speechEmpty: "没有识别到语音内容。",
    microphoneDenied: "麦克风权限未开启。请在浏览器地址栏中允许 Moros 使用麦克风。",
    speechFailed: "语音识别失败：{error}",
  },
  "zh-TW": {
    apiUnavailable: "Moros Web API 目前無法使用。",
    speechUnsupported: "目前瀏覽器不支援語音辨識，請使用最新版 Chrome 或桌面版。",
    speechEmpty: "沒有辨識到語音內容。",
    microphoneDenied: "尚未開啟麥克風權限。請在瀏覽器網址列允許 Moros 使用麥克風。",
    speechFailed: "語音辨識失敗：{error}",
  },
  en: {
    apiUnavailable: "Moros Web API is unavailable.",
    ...
    microphoneDenied: "Microphone access is disabled. Allow Moros to use the microphone in the browser address bar.",
    speechFailed: "Speech recognition failed: {error}",
  },
  de: {
    apiUnavailable: "Die Moros Web API ist nicht verfügbar.",
    ...
    microphoneDenied: "Der Mikrofonzugriff ist deaktiviert. Erlauben Sie Moros den Mikrofonzugriff in der Adressleiste.",
...
async function rpc<Method extends WebRpcMethod>(
  ...
  try {
    ...
    throw new Error(`Moros Web API returned ${response.status}.`);
  }
  if (!response.ok || !payload.ok) {
    throw new Error(payload.error ?? `Moros Web API returned ${response.status}.`);
  }
...
export function createWebApi(): MorosApi {
