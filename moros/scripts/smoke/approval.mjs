export async function runApprovalScenario({ app, page, shot }) {
  await app.evaluate(({ BrowserWindow }) => {
    BrowserWindow.getAllWindows()[0]?.webContents.send("agent:event", {
      ...
      request: {
        id: "smoke-approval",
        toolName: "bash",
        message: "Allow Moros to run a shell command?",
        detail: "bash\\necho smoke",
  ...
  const approvalActions = page.locator(".approval-request-actions button");
  const approvalActionLabels = (await approvalActions.allTextContents()).map((label) => label.trim());
  if (
    approvalActionLabels.length !== 3
    || !approvalActionLabels.some((label) => label.startsWith("Deny"))
    || !approvalActionLabels.some((label) => label.startsWith("Allow once"))
    || !approvalActionLabels.some((label) => label.startsWith("Always allow this session"))
  ) {
    throw new Error(`Inline approval actions are incomplete: ${JSON.stringify(approvalActionLabels)}`);
  }
