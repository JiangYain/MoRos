...
export type BackendOperationArguments<Method extends BackendMethod> =
  CompassBackendApi[Method] extends (...args: infer Args) => Promise<unknown> ? Args : never;

export type BackendOperationResult<Method extends BackendMethod> =
  CompassBackendApi[Method] extends (...args: never[]) => Promise<infer Result> ? Result : never;
...
function dependencyIdArg(args: readonly unknown[], index: number): Parameters<CompassBackendApi["installDependency"]>[0] {
  const value = args[index];
...
function oneDependencyId(args: readonly unknown[]): [Parameters<CompassBackendApi["installDependency"]>[0]] {
  argumentCount(args, 1);
  return [dependencyIdArg(args, 0)];
}

/**
 * The one source of truth for every backend operation transported by Compass.
...
export const BACKEND_OPERATION_SPECS = {
  ...
  importLegacyClientRegistry: {
    ...
    web: true,
    decode: oneString("serializedRegistry"),
  },
  saveClientProfile: {
    ipcChannel: "clients:save-profile",
    web: true,
    decode: (args) => {
      argumentCount(args, 1);
      const profile = normalizeClientProfileDraft(args[0]);
  ...
  assignSessionClient: {
    ...
    web: true,
    decode: twoStrings("sessionId", "clientName"),
  },
  ...
  selectDependencyExecutable: {
    ipcChannel: "dependencies:select-executable",
    web: true,
    decode: (args) => {
      argumentCount(args, 1, 2);
      return [dependencyIdArg(args, 0), optionalStringArg(args, 1, "path")];
    },
  },
  resetDependencyExecutable: {
    ipcChannel: "dependencies:reset-executable",
    web: true,
    decode: oneDependencyId,
  },
  setSkillEnabled: {
...
export function createBackendTransportClient(invoke: BackendTransportInvoker): CompassBackendApi {
...
export function createWebRpcHandlers(api: CompassBackendApi): WebRpcHandlers {
