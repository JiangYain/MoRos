...
function SkillArtwork({ name }: { name: string }): React.JSX.Element {
  return name === "phonak-target-control" ? <img className="settings-skill-artwork-image" src={phonakTargetAppIcon} alt="" /> : <Puzzle size={18} strokeWidth={1.55} aria-hidden="true" />;
}
        ...
        <div className="settings-skill-list" id="settings-skills-list">
          {visible.map((skill) => (
            <article className={`settings-skill-row${skill.enabled ? "" : " disabled"}`} key={skill.name}>
              <div className="settings-skill-icon"><SkillArtwork name={skill.name} /></div>
