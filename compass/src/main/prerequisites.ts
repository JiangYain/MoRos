...
function refreshAction(): RuntimePrerequisiteAction {
  return {
    id: REFRESH_PREREQUISITES_ACTION_ID,
    label: "刷新检测",
    description: "重新检查 Compass 当前能否找到可用的 Bash。",
  };
}

function installActions(): RuntimePrerequisiteAction[] {
  if (process.platform !== "win32") return [refreshAction()];
  return [
    {
      ...
      shell: {
        ...
        detail:
          process.platform === "win32"
            ? `Compass 需要先安装 Git Bash 或其他 bash.exe，Agent 的 shell 工具才能执行。${errorMessage(error)}`
            : errorMessage(error),
