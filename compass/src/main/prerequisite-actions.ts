...
export async function runPrerequisiteAction(
  ...
  const copy: Record<AppLanguage, { buttons: [string, string]; message: string; detail: string }> = {
    ...
    "zh-TW": {
      ...
      detail: `Compass 將開啟 PowerShell 執行：\n${action.command}\n\n安裝完成後回到 Compass，按一下「重新檢查」。`,
    },
    en: {
      buttons: ["Open install command", "Cancel"], message: "Install Git for Windows",
      detail: `Compass will open PowerShell and run:\n${action.command}\n\nAfter installation, return to Compass and select “Check again”.`,
    },
    de: {
      buttons: ["Installationsbefehl öffnen", "Abbrechen"], message: "Git for Windows installieren",
      detail: `Compass öffnet PowerShell und führt Folgendes aus:\n${action.command}\n\nKehren Sie danach zu Compass zurück und wählen Sie „Erneut prüfen“.`,
    },
  };
