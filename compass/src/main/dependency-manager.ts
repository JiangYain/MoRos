...
export type { InstalledProgramRecord } from "./dependencies/inventory.ts";
export {
...
export interface DependencyManagerOptions {
  ...
  setExecutablePath: (dependencyId: DependencyId, path?: string) => void | Promise<void>;
...
export class DependencyManager {
  ...
  async snapshot(
    ...
    if (shouldRefreshDependencyInventory(Boolean(this.cachedItems), this.cachedAt, Date.now(), options)) {
      this.cachedItems = await inspectDependencyResources(
        prerequisites,
  ...
  async setExecutable(dependencyId: DependencyId, path?: string): Promise<void> {
    ...
    if (!path) {
      await this.persistExecutablePath(dependencyId, undefined);
    ...
    await this.persistExecutablePath(dependencyId, resolvedPath);
    this.invalidateInventory();
  }
